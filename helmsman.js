...
function Helmsman(options) {
  ...
  if (!options.prefix && path.basename(process.argv[1]) !== path.basename(require.main.filename)) {
    this.prefix = path.basename(require.main.filename, path.extname(require.main.filename));
  } else {
    this.prefix = options.prefix || path.basename(process.argv[1], path.extname(process.argv[1]));
  }
  ...
  this.localFiles.forEach(function(file) {
    ...
    if (extension === '' || extension === '.js') {
      try {
        ...
        if (extension === '.js') {
          util.error(util.format('The file "%s" did not load correctly: %s', file, e).red);
          process.exit(1);
        }
      }
    }
